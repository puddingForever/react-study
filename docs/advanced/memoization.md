...
- Button 이 React.memo로 감싸져있음 ( Props가 변하지 않으면 다시 랜더링 안함 ) 
- 그런데 , onClick을 계속 클릭하기 때문에 , props가 바뀌었다고 판단해서 재랜더링함 
- 이때 useCallback 함수를 사용하여, 재랜더링 방지
- [] -> 의존성없음 -> 컴포넌트가 다시 랜더링되도 항상 같은 함수 참조 사용
- 결과 : React.memo 자식이 불필요하게 다시 랜더링되지 않음 
